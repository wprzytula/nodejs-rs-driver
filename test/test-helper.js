...
const ccm = {
     ...
     */
    startAll: function (nodeLength, options, callback) {
        ...
        utils.series(
            [
                ...
                function (next) {
                    ...
                    self.exec(
                        populate,
                        helper.waitCallback(options.sleep, next),
                    );
                },
                function (next) {
                ...
                function (next) {
                    ...
                    self.exec(
                        ["setworkload", options.workloads.join(",")],
                        next,
                    );
                },
                function (next) {
                    ...
                    if (
                        helper.isWin() &&
                        helper.isCassandraGreaterThan("2.2.4")
                    ) {
     ...
     * @param {Function} callback
     */
    setWorkload: function (nodeIndex, workloads, callback) {
...
const ads = {
    _spawnAndWait: function (processName, params, cb) {
        if (!cb) {
        ...
        const p = spawn(processName, params, {
            env: Object.assign(
                { KRB5_CONFIG: this.getKrb5ConfigPath() },
                process.env,
            ),
        });
    ...
    _exec: function (processName, params, callback) {
        if (params.length === 0) {
     ...
     */
    getKeytabPath: function (username) {
...
const helper = {
    ...
    assertInstanceOf: function (instance, constructor) {
        assert.notEqual(
            instance,
            null,
            "Expected instance, obtained " + instance,
        );
        assert.ok(
    ...
    assertProperties: (value, expectedProperties, strictEquality) => {
        ...
        if (properties.length === 0) {
            throw new Error(
                "expectedProperties should be defined as an object",
            );
        }
        ...
        properties.forEach((key) =>
            assertFn(value[key], expectedProperties[key]),
        );
    },
    ...
    wait: {
        ...
        forAllNodesDown: async function (
            client,
            maxAttempts = 500,
            delay = 20,
        ) {
    ...
    queries: {
        ...
        basicNoResults:
            "SELECT key from system.local WHERE key = 'not_existent'",
    },
    getPoolingOptions: function (localLength, remoteLength, heartBeatInterval) {
        ...
        pooling.coreConnectionsPerHost[types.distance.remote] =
            remoteLength || 1;
        ...
        return new Promise((resolve, reject) => {
            const req = http.get(
                ...
                (res) => {
                    ...
                    res.on("data", (chunk) => (data += chunk.toString())).on(
                        "end",
                        () => {
                            if (res.statusCode !== 200) {
                                return reject(
                                    new Error(
                                        `Obtained http status ${res.statusCode}`,
                                    ),
                                );
                            }
