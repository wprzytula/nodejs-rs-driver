...
const ExecutionProfile =
    require("../../../lib/execution-profile.js").ExecutionProfile;
...
describe("Client @SERVER_API", function () {
    ...
    describe("#execute(query, params, {prepare: 0}, callback)", function () {
                    ...
                    function ensureProfileCLUsed(next) {
                        client.execute(
                            ...
                            function (err, result) {
                                assert.ifError(err);
                                assert.strictEqual(
                                    result.info.achievedConsistency,
        ...
        it("should return rows that are serializable to json", function (done) {
            ...
            utils.series(
                [
                    ...
                    function select(next) {
                        const query = util.format(
                            "SELECT id, timeuuid_sample, inet_sample, bigint_sample, decimal_sample from %s WHERE id = %s",
                            table,
                            id,
                        );
                        client.execute(query, function (err, result) {
        ...
        vit(
            ...
            function (done) {
                ...
                const client = newInstance({
                    profiles: [
                        new ExecutionProfile("cl", {
                            serialConsistency: types.consistencies.one,
                        }),
                    ],
                });
                const id = types.Uuid.random();
                utils.series(
                    [
                        function insertWithProfileSerialCL(next) {
                            const query = util.format(
                                "INSERT INTO %s (id, text_sample) VALUES (?, ?) IF NOT EXISTS",
                                table,
                            );
                            client.execute(
                                query,
                        ...
                        function select(next) {
                            const query = util.format(
                                "SELECT id, text_sample from %s WHERE id = ?",
                                table,
                            );
                            client.execute(query, [id], function (err, result) {
                                const row = result.first();
        ...
        vit("2.1", "should support protocol level timestamp", function (done) {
            ...
            utils.series(
                [
                    ...
                    function select(next) {
                        const query = util.format(
                            "SELECT id, text_sample, writetime(text_sample) from %s WHERE id = ?",
                            table,
                        );
                        client.execute(query, [id], function (err, result) {
                            const row = result.first();
                            ...
                            assert.strictEqual(
                                row["writetime(text_sample)"].toString(),
                                timestamp.toString(),
                            );
                            next();
                        });
        ...
        vit(
            ...
            function (done) {
                ...
                client.on("log", function (level, className, message) {
                    ...
                    if (
                        message.indexOf("batch") >= 0 &&
                        message.indexOf("exceeding")
                    ) {
                        loggedMessage = true;
                    }
                });
        ...
        it("should support buffer as input for any data type", () => {
            ...
            return Promise.all(
                [false, true].map((prepare) =>
                    client
                        .execute(insertQuery, params, { prepare })
                        ...
                        .then((rs) => {
                            ...
                            assert.strictEqual(
                                row["id"].toString(),
                                new types.Uuid(buffer16).toString(),
                            );
                            assert.strictEqual(
                                row["text_sample"],
                                buffer4.toString("utf8"),
                            );
                            assert.strictEqual(row["int_sample"], 1);
                            assert.strictEqual(
                                row["bigint_sample"].toString(),
                                "0",
                            );
                            assert.strictEqual(
                                row["float_sample"],
                                buffer4.readFloatBE(0),
                            );
                            assert.strictEqual(row["double_sample"], 0);
                            assert.strictEqual(
                                row["inet_sample"].toString(),
                                "0.0.0.1",
                            );
        ...
        describe("with udt and tuple", function () {
            ...
            const selectQuery =
                "SELECT id, phone_col, address_col FROM tbl_udts WHERE id = %s";
            ...
            vit("2.1", "should retrieve column information", function (done) {
                ...
                client.execute(
                    ...
                    function (err, result) {
                        ...
                        assert.strictEqual(
                            phoneInfo.fields[0].type.code,
                            types.dataTypes.varchar,
                        );
                        assert.strictEqual(
                            phoneInfo.fields[2].name,
                            "country_code",
                        );
                        assert.strictEqual(
                            phoneInfo.fields[2].type.code,
                            types.dataTypes.int,
                        );
                        assert.strictEqual(
                            result.columns[2].type.code,
                        ...
                        assert.strictEqual(
                            addressInfo.fields[0].name,
                            "street",
                        );
                        assert.strictEqual(addressInfo.fields[1].name, "ZIP");
                        assert.strictEqual(
                            addressInfo.fields[1].type.code,
                            types.dataTypes.int,
                        );
                        assert.strictEqual(
                            addressInfo.fields[2].name,
                            "phones",
                        );
                        assert.strictEqual(
                            addressInfo.fields[2].type.code,
                            types.dataTypes.set,
                        );
                        assert.strictEqual(
                            addressInfo.fields[2].type.info.code,
                            types.dataTypes.udt,
                        );
                        ...
                        assert.strictEqual(
                            subPhone.fields[0].type.code,
                            types.dataTypes.varchar,
                        );
                        assert.strictEqual(subPhone.fields[1].name, "number");
                        assert.strictEqual(
                            subPhone.fields[1].type.code,
                            types.dataTypes.varchar,
                        );
                        assert.strictEqual(
                            subPhone.fields[2].name,
                            "country_code",
                        );
                        assert.strictEqual(
                            subPhone.fields[2].type.code,
                            types.dataTypes.int,
                        );
                        done();
                    },
            ...
            /* vit(
                ...
                function (done) {
                    ...
                    const address = {
                        ...
                        ZIP: 90987,
                        phones: [
                            { alias: "personal2", number: "555 0001" },
                            { alias: "work2" },
                        ],
                    };
                    const id = types.Uuid.random();
                    ...
                    [
                        function insert(next) {
                            ...
                            client.execute(
                                query,
                                [id, tuple],
                                { hints: [null, "tuple<text, int,blob>"] },
                                next,
                            );
                        },
                        function select(next) {
                            client.execute(
                                util.format(
                                    "SELECT * FROM tbl_tuples WHERE id = ?",
                                ),
                                [id],
                                function (err, result) {
                                    assert.ifError(err);
                                    ...
                                    assert.strictEqual(
                                        row["tuple_col"].get(2).toString("hex"),
                                        "0f0f",
                                    );
                                    next();
                                },
            ...
            vit("2.2", "should allow insertions as json", function (done) {
                ...
                const o = {
                    ...
                    address_col: {
                        street: "whatever",
                        phones: [{ alias: "main", number: "0000212123" }],
                    },
                };
                utils.series(
                    [
                        ...
                        function select(next) {
                            ...
                            client.execute(
                                ...
                                function (err, result) {
                                    ...
                                    assert.strictEqual(
                                        row["address_col"].street,
                                        o.address_col.street,
                                    );
                                    assert.strictEqual(
                                        row["address_col"].toString(),
                                        o.address_col.toString(),
                                    );
                                    next();
                                },
                            );
        ...
        /* describe("with named parameters", function () {
            vit("2.1", "should allow named parameters", function (done) {
                ...
                const values = {
                    id: types.Uuid.random(),
                    myText: "hello",
                    myBigInt: types.Long.fromNumber(2),
                };
                const client = setupInfo.client;
                client.execute(query, values, function (err) {
                    ...
                    verifyRow(
                        table,
                        values.id,
                        "text_sample, bigint_sample",
                        [values.myText, values.myBigInt],
                        done,
                    );
                });
            });
            vit("2.1", "should use parameter hints", function (done) {
                const query = util.format(
                    "INSERT INTO %s (id, int_sample, float_sample) VALUES (:id, :myInt, :myFloat)",
                    table,
                );
                const values = {
                    id: types.Uuid.random(),
                    myInt: 100,
                    myFloat: 2.0999999046325684,
                };
                const client = setupInfo.client;
                client.execute(
                    query,
                    values,
                    {
                        hints: {
                            myFloat: "float",
                            myInt: { code: types.dataTypes.int },
                        },
                    },
                    function (err) {
                        assert.ifError(err);
                        verifyRow(
                            table,
                            values.id,
                            "int_sample, float_sample",
                            [values.myInt, values.myFloat],
                            done,
                        );
                    },
                );
            ...
            vit(
                "2.1",
                "should allow parameters with different casings",
                function (done) {
                    const query = util.format(
                        "INSERT INTO %s (id, text_sample, list_sample2) VALUES (:ID, :MyText, :mylist)",
                        table,
                    );
                    const values = {
                        id: types.Uuid.random(),
                    ...
                    client.execute(
                        query,
                        values,
                        { hints: { myLIST: "list<int>" } },
                        function (err) {
                            assert.ifError(err);
                            verifyRow(
                                table,
                                values.id,
                                "text_sample, list_sample2",
                                [values.mytext, values.myLIST],
                                done,
                            );
                        },
                    );
        ...
        describe("with smallint and tinyint", function () {
            ...
            const selectQuery =
                "SELECT id, smallint_sample, tinyint_sample, text_sample FROM tbl_smallints WHERE id = %s";
            before(function (done) {
                const client = setupInfo.client;
                utils.series(
                    [
            ...
            vit(
                "2.2",
                "should retrieve smallint and tinyint values as Number",
                function (done) {
            ...
            vit(
                "2.2",
                "should encode and decode smallint and tinyint values as Number",
                function (done) {
                    const client = setupInfo.client;
                    ...
                    client.execute(
                        query,
                        [id, 10, 11, "another text"],
                        { hints: [null, "smallint", "tinyint"] },
                        function (err) {
                            assert.ifError(err);
                            client.execute(
                                util.format(selectQuery, id),
                                function (err, result) {
                                    assert.ifError(err);
                                    assert.ok(result);
                                    assert.ok(result.rowLength);
        ...
        describe("with date and time types", function () {
            ...
            const insertQuery =
                "INSERT INTO tbl_datetimes (id, date_sample, time_sample) VALUES (?, ?, ?)";
            const selectQuery =
                "SELECT id, date_sample, time_sample FROM tbl_datetimes WHERE id = ?";
            before(function (done) {
                const query =
            ...
            vit(
                "2.2",
                "should encode and decode date and time values as LocalDate and LocalTime",
                function (done) {
                    const values = [
                        [
                            types.Uuid.random(),
                            new LocalDate(1969, 10, 13),
                            new LocalTime(types.Long.fromString("0")),
                        ],
                        [
                            types.Uuid.random(),
                            new LocalDate(2010, 4, 29),
                            LocalTime.fromString("15:01:02.1234"),
                        ],
                        [
                            types.Uuid.random(),
                            new LocalDate(2005, 8, 5),
                            LocalTime.fromString("01:56:03.000501"),
                        ],
                        [
                            types.Uuid.random(),
                            new LocalDate(1983, 2, 24),
                            new LocalTime(
                                types.Long.fromString("86399999999999"),
                            ),
                        ],
                        [
                            types.Uuid.random(),
                            new LocalDate(-2147483648),
                            new LocalTime(
                                types.Long.fromString("6311999549933"),
                            ),
                        ],
                    ];
                    const client = setupInfo.client;
        ...
        describe("with json support", function () {
            ...
            vit(
                "2.2",
                "should allow insert of all ECMAScript types as json",
                function (done) {
                    const client = setupInfo.client;
                    const o = {
                        id: types.Uuid.random(),
                            ...
                            function select(next) {
                                ...
                                client.execute(
                                    ...
                                    function (err, result) {
                                        ...
                                        assert.strictEqual(
                                            row["timestamp_sample"].getTime(),
                                            o.timestamp_sample.getTime(),
                                        );
                                        ...
                                        assert.strictEqual(
                                            row["list_sample"].toString(),
                                            o.list_sample.toString(),
                                        );
                                        assert.strictEqual(
                                            row["list_sample2"].toString(),
                                            o.list_sample2.toString(),
                                        );
                                        assert.strictEqual(
                                            row["set_sample"].toString(),
                                            o.set_sample.toString(),
                                        );
                                        next();
                                    },
                                );
        ...
        vdescribe("2.0", "with lightweight transactions", function () {
            ...
            [
                [
                    "is not a conditional update",
                    true,
                    "INSERT INTO %s (id, text_sample) VALUES (?, ?)",
                    [id, "val"],
                ],
                [
                    "is a conditional update and it was applied",
                    true,
                    "INSERT INTO %s (id, text_sample) VALUES (?, ?) IF NOT EXISTS",
                    [types.Uuid.random(), "val2"],
                ],
                [
                    "is a conditional update and it was not applied",
                    false,
                    "INSERT INTO %s (id, text_sample) VALUES (?, ?) IF NOT EXISTS",
                    [id, "val"],
                ],
                ...
                context("when it " + item[0], function () {
                    it(
                        "should return a ResultSet with wasApplied set to " +
                            item[1],
                        function (done) {
                            client.execute(
                                util.format(item[2], table),
                                item[3],
                                function (err, result) {
                                    assert.ifError(err);
                                    assert.strictEqual(
...
function verifyRow(table, id, fields, values, callback) {
    const client = newInstance();
    client.execute(
        util.format("SELECT %s FROM %s WHERE id = %s", fields, table, id),
        function (err, result) {
            assert.ifError(err);
            const row = result.first();
